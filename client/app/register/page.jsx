...
export default function RegisterPage() {
  ...
  const [step, setStep]   = useState(1); // 1: account, 2: fighter stats
    ...
    <div style={s.root}>
      <div style={s.left}>
        ...
        <div style={s.stepDesc}>
          {step === 1
            ? 'Create your account. Already have one? Sign in with Google in one tap.'
            : 'Your physical stats determine your weight class and matchmaking bracket.'}
        </div>
      </div>
      ...
      <div style={s.right}>
        <div style={s.formBox}>
          <div style={s.formTitle}>
            {step === 1 ? 'CREATE ACCOUNT' : 'FIGHTER STATS'}
          </div>
          ...
          {step === 1 && (
            <>
              ...
              <form onSubmit={handleNext} style={s.form}>
                <Field label="USERNAME" type="text" placeholder="bruiser_99"
                  value={form.username}
          ...
          {step === 2 && (
            <form onSubmit={handleSubmit} style={s.form}>
              <div style={s.rowFields}>
                <Field label="HEIGHT (CM)" type="number" placeholder="175"
                  value={form.heightCm}
                  onChange={v => setForm({ ...form, heightCm: v })} required />
                <Field label="WEIGHT (KG)" type="number" placeholder="70"
                  value={form.weightKg}
                  onChange={v => setForm({ ...form, weightKg: v })} required />
              </div>
              <Field label="REACH (CM) — OPTIONAL" type="number" placeholder="180"
                value={form.reachCm}
                onChange={v => setForm({ ...form, reachCm: v })} required={false} />

              <div style={s.weightClassHint}>
                Weight class is auto-assigned from your weight.
              </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      <div style={{
        ...
        border: `2px solid ${active || done ? '#cc2200' : '#2a2a2a'}`,
      ...
      <span style={{
        color: active ? '#e8e4dc' : done ? '#cc2200' : '#3a3a3a',
...
const s = {
  ...
  formTitle: { fontFamily: "'Bebas Neue', 'Arial Black', sans-serif", fontSize: '28px', color: '#e8e4dc', letterSpacing: '0.08em', marginBottom: '4px' },
  ...
  submitBtn: { background: '#cc2200', border: 'none', borderRadius: '6px', color: '#e8e4dc', cursor: 'pointer', fontFamily: "'Bebas Neue', 'Arial Black', sans-serif", fontSize: '18px', letterSpacing: '0.1em', padding: '13px' },
  rowFields: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' },
  weightClassHint: { color: '#3a3a3a', fontSize: '11px', letterSpacing: '0.02em' },
  btnRow: { display: 'flex', gap: '10px', marginTop: '4px' },
  backBtn: { background: 'transparent', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#4a4a4a', cursor: 'pointer', fontFamily: "'Bebas Neue', 'Arial Black', sans-serif", fontSize: '16px', letterSpacing: '0.08em', padding: '13px 16px' },
