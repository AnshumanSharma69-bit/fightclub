...
// ─── GET /api/zone/all ────────────────────────────────────────────────────────
// Returns all zones with their current holder info
// Used by the map to draw overlays
...
// Returns fighters sorted by number of badges held
const getLeaderboard = async (req, res, next) => {
  try {
    const fighters = await Fighter.find({
      'badgesEarned.0': { $exists: true }, // only fighters with at least 1 badge
    })
      .populate('userId', 'username avatarUrl')
      .populate('badgesEarned', 'name badgeEmoji')
      .sort({ 'badgesEarned.length': -1, eloRating: -1 })
      .limit(20);
...
// ─── GET /api/zone/mine ───────────────────────────────────────────────────────
// Returns zones where the logged-in fighter is the holder
...
// ─── POST /api/zone/seed ──────────────────────────────────────────────────────
// Seeds initial zones — run once to populate DB with city boundaries
// Protected: only works in development
